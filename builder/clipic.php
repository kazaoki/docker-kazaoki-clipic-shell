...
$color_map = imagecreatetruecolor(16, 16);
imagetruecolortopalette($color_map, false, 255);
for($i=0; $i<256; $i++) {
	if(!preg_match('/^([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i', str_pad($color_index[$i], 6, '0', STR_PAD_LEFT), $rgb)) {
		fputs(STDERR, "Can't convert color index: [${i}]${color_index[$i]}\n");
	}
	imagecolorallocate($color_map, hexdec($rgb[1]), hexdec($rgb[2]), hexdec($rgb[3]));
	imagecolorset($color_map, $i, hexdec($rgb[1]), hexdec($rgb[2]), hexdec($rgb[3]));
}
